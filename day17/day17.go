...
func partTwo(lines []string) {
	...
	xmin, xmax, ymin, ymax, zmin, zmax, wmin, wmax := 0, len(lines[0])-1, 0, len(lines)-1, 0, 0, 0, 0
	...
	for i := 0; i < 6; i++ {
		...
		xmint, xmaxt, ymint, ymaxt, zmint, zmaxt, wmint, wmaxt := xmin, xmax, ymin, ymax, zmin, zmax, wmin, wmax
			...
			for z := zmin - 1; z <= zmax+1; z++ {
				...
				for y := ymin - 1; y <= ymax+1; y++ {
					...
					for x := xmin - 1; x <= xmax+1; x++ {
							...
							for zi := z - 1; zi <= z+1; zi++ {
								for yi := y - 1; yi <= y+1; yi++ {
									for xi := x - 1; xi <= x+1; xi++ {
										...
										if !(xi == x && yi == y && z == zi && w == wi) && isActive2(xi, yi, zi, wi, cubes) {
											nActiveCnt++
										}
									}
								}
						...
						if isActive2(x, y, z, w, cubes) {
							if nActiveCnt < 2 || nActiveCnt > 3 {
								toDeactivate[getKey2(x, y, z, w)] = true
							}
						} else if nActiveCnt == 3 {
							// If a cube is inactive but exactly 3 of its neighbors are active, the cube becomes active. Otherwise, the cube remains inactive.
							toActivate[getKey2(x, y, z, w)] = true
		...
		// may need to reset max/min
		xmin, xmax, ymin, ymax, zmin, zmax, wmin, wmax = xmint, xmaxt, ymint, ymaxt, zmint, zmaxt, wmint, wmaxt

		fmt.Println("Total active: ", len(cubes))
	}
}

func getKey2(x int, y int, z int, w int) string {
	return fmt.Sprintf("%d|%d|%d|%d", x, y, z, w)
}

func isActive2(x int, y int, z int, w int, cubes map[string]bool) bool {
	_, exists := cubes[getKey2(x, y, z, w)]
	return exists
}
